...
def main(args):
    ...
    tracking_uri = args.mlflow_uri or f"sqlite:///{db_path}" 
    ...
    with mlflow.start_run():
        ...
        mlflow.log_param("data_quality_pass", int(is_valid))
        ...
        feature_cols = df_enc.drop(columns=[target]).columns.tolist()
        ...
        model_info = mlflow.xgboost.log_model(xgb_model=model, name="model", input_example=X_train.head(5))

        print(f"Model saved to MLflow: {model_info.model_uri}")
...
if __name__ == "__main__":
    ...
    p.add_argument("--mlflow_uri", type=str, default=None, help="override MLflow tracking URI, else uses project_root/mlruns")
